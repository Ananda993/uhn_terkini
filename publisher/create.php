...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare('INSERT INTO informasi (judul, slug, deskripsi, gambar, deadline, category_id, user_id, status) VALUES (?, ?, ?, ?, ?, ?, ?, "pending")');
        $userId = (int)$_SESSION['user_id'];
        $stmt->bind_param('sssssii', $judul, $slug, $cleanHtml, $gambarName, $deadline, $categoryId, $userId);
        $stmt->execute();
        $stmt->close();
