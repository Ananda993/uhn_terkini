...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare('INSERT INTO informasi (judul, slug, deskripsi, gambar, deadline, category_id, user_id, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
        $userId = (int)($_SESSION['user_id'] ?? 1);
        $stmt->bind_param('sssssiis', $judul, $slug, $cleanHtml, $gambarName, $deadline, $categoryId, $userId, $status);
        $stmt->execute();
        $stmt->close();
