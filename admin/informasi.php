...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'set_urgent' && $id) {
        $conn->query("UPDATE informasi SET is_urgent=1 WHERE id=$id");
        header('Location: ' . ($_SERVER['HTTP_REFERER'] ?? '/admin/informasi.php'));
        exit;
    }
    if ($action === 'unset_urgent' && $id) {
        $conn->query("UPDATE informasi SET is_urgent=0 WHERE id=$id");
        header('Location: ' . ($_SERVER['HTTP_REFERER'] ?? '/admin/informasi.php'));
        exit;
    }
}
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="max-width:250px;">
                                ...
                                <div style="font-size:0.6875rem; color:#94a3b8; margin-top:0.125rem;">Sumber: <?= htmlspecialchars($item['sumber']) ?></div>
                                <?php endif; ?>
                            ...
                            <td>
                                <span class="badge-<?= $item['is_urgent'] ? 'pending' : 'rejected' ?>">
                            ...
                            <td>
                                <div style="display:flex; gap:0.375rem;">
                                    ...
                                    <form method="POST" style="display:inline;">
                                        <input type="hidden" name="action" value="unset_urgent">
                                        <input type="hidden" name="id" value="<?= $item['id'] ?>">
                                        <button type="submit" class="btn-action btn-reject" title="Jadikan Normal"><i class="fas fa-arrow-down"></i></button>
                                    </form>
                                    <?php else: ?>
                                    <form method="POST" style="display:inline;">
                                        <input type="hidden" name="action" value="set_urgent">
                                        <input type="hidden" name="id" value="<?= $item['id'] ?>">
                                        <button type="submit" class="btn-action btn-approve" title="Naikkan Urgensi"><i class="fas fa-arrow-up"></i></button>
                                    </form>
                                    <?php endif; ?>
...
@media (max-width: 768px) {
    ...
    .admin-table thead th:nth-child(5), .admin-table tbody td:nth-child(5) { display: none; }
}
</style>
</body>
</html>
